...
from py02_apexrch_wq_funs import *
###########################################################################
...
index = pd.date_range(df_start_time, df_end_time, freq = "1M")
dateformat = "%#m/%#d/%Y"
column = ["ObsNH3N", "SimNH3N",
          "ObsNO3N", "SimNO3N",
          "ObsTN", "SimTN",
          "ObsTP", "SimTP",]
...
for stnidx in range(len(stnnumlst)):
    ...
    lif_obs = 0
    obswqname = "%sS%s_1216.csv" %(obsinfd, stnnumlst[stnidx])
    lif_obs = read_text(obswqname)
    
    # Processing data lines
    lif_obs = reformat_obslines(lif_obs)

    lif_sim = 0
    lif_sim = read_text(siminfd + "allvarmon_%s.apexsimrch" 
                        %(stnnumlst[stnidx]))
    ...
    # Generate dataframe 
    df_mon = generate_timeframe(df_start_time,
                                        df_end_time,
                                        index,
                                        column)
